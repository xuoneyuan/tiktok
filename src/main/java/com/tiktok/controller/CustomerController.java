...
import com.tiktok.entity.vo.UpdateUserVO;
...
public class CustomerController {
    ...
    public R saveOrUpdateFavorites(@RequestBody @Validated Favorites favorites){
        ...
        Long count = favoritesService.count(new LambdaQueryWrapper<Favorites>().eq(Favorites::getName,favorites.getName()).eq(Favorites::getUserId,userId).ne(Favorites::getId,favorites.getId()));
        if(count == 1){
    ...
    public R subscribe(@RequestParam(required = false) String types){
        ...
        if(!ObjectUtils.isEmpty(types)){
            for(String type : types.split(",")){
                hashSet.add(Long.parseLong(type));
            }
            msg = "订阅成功";
        }
